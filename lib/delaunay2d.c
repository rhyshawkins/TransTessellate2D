...
#include "rjmcmc_defines.h"

#define TRIANGLE_INCR 1024
#define DLIST_INCR 16

//
// Experiments show that these values are a function of the number of points in a
// region. For random models with 10,000 points (more than this code is expected to be
// used for), we obtain maximums of 24 and 52 respectively. We've used the next power
// of 2 to give a small buffer in case this is exceeded.
//
#define MAXEDGES 32
#define MAXTRIANGLEEDGES 64
...
struct _point {
  ...
  int nweights;
  int weight_refs[MAXTRIANGLEEDGES];
  double dxweights[MAXTRIANGLEEDGES];
  double dyweights[MAXTRIANGLEEDGES];
...
struct _triangle {
  ...
  int nweights; 
  int weight_refs[MAXTRIANGLEEDGES];
  double weights[20][MAXTRIANGLEEDGES];
};
...
struct _delaunay2d {
  ...
  point_t *p;    /* Contains list of points */
  edgelist_t *e; /* Contains indice list of neigboring vertices to p */
  int np;
...
static double dist2(const delaunay2d_t *d,
		    int pi,
		    double px,
		    double py);

static int ct_add_weight(int *nweights,
			 int *weight_refs,
			 double *weights,
			 int idx,
			 double weight);

static int ct_initialize_weight(int nweights,
				int *weight_refs,
				double *weights,
				int idx,
				double weight);

static int ct_combine_weight(int nweights,
			     double *weights,
			     const double *add_weights,
			     double scale);

static int ct_initialize_weight_with_three(int nweights,
					   double *weights,
		  ...
		  double ymax)
{
  ...
  RJMCMC_INTCHECKPTR(add_point(d, xmin, ymin, 0.0), "delaunay2d_create: failed to add point\n");
  RJMCMC_INTCHECKPTR(add_point(d, xmin, ymax, 0.0), "delaunay2d_create: failed to add point\n");
  RJMCMC_INTCHECKPTR(add_point(d, xmax, ymax, 0.0), "delaunay2d_create: failed to add point\n");
  RJMCMC_INTCHECKPTR(add_point(d, xmax, ymin, 0.0), "delaunay2d_create: failed to add point\n");
...
{
  memset(d->p, 0, sizeof(point_t) * d->maxpoints);
  d->np = 0;
  d->nt = 0;
  
  add_point(d, d->xmin, d->ymin, 0.0);
  add_point(d, d->xmin, d->ymax, 0.0);
  add_point(d, d->xmax, d->ymax, 0.0);
  add_point(d, d->xmax, d->ymin, 0.0);

  add_triangle(d, 0, 1, 2, -1, -1, 1);
  add_triangle(d, 0, 2, 3, 0, -1, -1);
}
		...
		const char *filename)
{
  ...
  fprintf(fp, "%d %d\n", d->maxpoints, d->np);
  fprintf(fp, "%16.9e %16.9e %16.9e %16.9e\n", d->xmin, d->xmax, d->ymin, d->ymax);

  for (i = 0; i < d->np; i ++) {
    fprintf(fp, "%16.9e %16.9e %16.9e\n", d->p[i].x, d->p[i].y, d->p[i].z);
  }
		   ...
		   bbox2d_t *bound)
{
  ...
  if (ei >= 0) {
    ...
    if (oedge < 0) {
      ...
      RJMCMC_CONDITIONCHECKINT(add_point(d, x, y, z) < 0, 
...
int delaunay2d_nearest_from(const delaunay2d_t *d,
			    int p0,
			    int include_corners,
			    double px,
			    double py)
{
  ...
  if (include_corners == 0 &&
    ...
    if (best_pi < 0) {
      fprintf(stderr, "delaunay2d_nearest_from: unable to find nearest point to %f %f\n", px, py);
			  ...
			  double *z)
{
  ...
  //RJMCMC_CONDITIONCHECKINT(i >= d->np, "delaunay2d_point_of_index: invalid index\n");
  if (i >= d->np) {
    fprintf(stderr, "delaunay2d_point_of_index: invalid index %d (%d)\n",
	    i, d->np);
    return -1;
  }
...
delaunay2d_set_value_of_index(const delaunay2d_t *d,
			      ...
			      double z)
{
  RJMCMC_CONDITIONCHECKINT(d == NULL, "delaunay2d_set_value_of_index: NULL delaunay\n");
  if (i >= d->np) {
    fprintf(stderr, "delaunay2d_set_value_of_index: invalid index %d (%d)\n",
	    i, d->np);
    return -1;
  }
			    ...
			    double *z)
{
  ...
  t = delaunay2d_nearest_from(d, t0, 0, px, py);
  if (t < 0) {
    return t;
  }
...
int
delaunay2d_ct_compute_node_gradients(delaunay2d_t *d)
{
  int i;
  int j;
  ...
  double dxi[MAXTRIANGLEEDGES];
  double dyi[MAXTRIANGLEEDGES];
  double dzi[MAXTRIANGLEEDGES];
  ...
  for (i = 0; i < d->np; i ++) {
    ...
    /*
     * Zero everything for this point
     */
    d->p[i].nweights = 0;
    for (j = 0; j < MAXTRIANGLEEDGES; j ++) {
      d->p[i].dxweights[j] = 0.0;
      d->p[i].dyweights[j] = 0.0;
    }

    /*
     * Loop through edges of this point
     */
    A = 0.0;
    B = 0.0;
    C = 0.0;
    ...
    for (j = 0; j < d->e[i].n; j ++) {
      ...
      dxi[j] = d->p[d->e[i].edge[j]].x - d->p[i].x;
      dyi[j] = d->p[d->e[i].edge[j]].y - d->p[i].y;
      dzi[j] = d->p[d->e[i].edge[j]].z - d->p[i].z;
    ...
    /*
     * Solve least squares (choosing better conditioned solution)
     */
    if (A > B) {
      d->p[i].dy = (A*by - C*bx)/(A*B - C*C);
      d->p[i].dx = (bx - C*d->p[i].dy)/A;
    } else {
      d->p[i].dx = (B*bx - C*by)/(B*A - C*C);
      d->p[i].dy = (by - C*d->p[i].dx)/B;
    }
    ...
    if (A > B) {
      ...
      wx = 0.0;
      for (j = 0; j < d->e[i].n; j ++) {
	wy += -dyi[j] * A/(A*B - C*C) + dxi[j] * C/(A*B - C*C);
	wx += -dxi[j]/A;
      }

      ct_add_weight(&(d->p[i].nweights),
		    d->p[i].weight_refs,
		    d->p[i].dyweights,
		    i,
		    wy);
      ...
      /* wrt to neighboring vertices */
      for (j = 0; j < d->e[i].n; j ++) {

	wy = A/(A*B - C*C)*dyi[j] - C/(A*B - C*C)*dxi[j];
	wx = dxi[j]/A - wy*C/A;
	...
	ct_add_weight(&(d->p[i].nweights),
      ...
      for (j = 0; j < d->e[i].n; j ++) {
	wx += -dxi[j] * B/(B*A - C*C) + dyi[j] * C/(B*A - C*C);
	wy += -dyi[j]/B;
      }
	...
	wx = B/(B*A - C*C)*dxi[j] - C/(B*A - C*C)*dyi[j];
	wy = dyi[j]/B - wx*C/B;
...
delaunay2d_ct_update(delaunay2d_t *d)
{
  ...
  double tdxweights[3][MAXTRIANGLEEDGES];
  double tdyweights[3][MAXTRIANGLEEDGES];

  if (delaunay2d_ct_compute_node_gradients(d) < 0) {
    return -1;
  }
  ...
  for (i = 0; i < d->nt; i ++) {
    ...
    /*
     * Build triangle weights and triangle dependency indices by combining
     * weights for three vertices
     */
    d->t[i].nweights = 0;
    for (j = 0; j < 3; j ++) {
      for (k = 0; k < MAXTRIANGLEEDGES; k ++) {
	tdxweights[j][k] = 0.0;
	tdyweights[j][k] = 0.0;
      }
    }
    ...
    for (j = 0; j < 3; j ++) {
      ...
      for (k = 0; k < d->p[d->t[i].v[j]].nweights; k ++) {

	ct_add_weight(&d->t[i].nweights,
		      d->t[i].weight_refs,
		      tdxweights[j],
		      d->p[d->t[i].v[j]].weight_refs[k],
		      d->p[d->t[i].v[j]].dxweights[k]);

	ct_add_weight(&d->t[i].nweights,
		      d->t[i].weight_refs,
		      tdyweights[j],
		      d->p[d->t[i].v[j]].weight_refs[k],
		      d->p[d->t[i].v[j]].dyweights[k]);

      }
    }

    /*
     * Dependence on node gradients
     */

    /* C4 */
    d->t[i].ct[4] = ((x2 - x1) * z1x + (y2 - y1) * z1y)/3.0 + z1;

    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[4],
			 d->t[i].v[0], 1.0);
    ct_combine_weight(d->t[i].nweights,
		      d->t[i].weights[4],
		      tdxweights[0],
		      (x2 - x1)/3.0);
    ct_combine_weight(d->t[i].nweights,
    ...
    /* C10 */
    d->t[i].ct[10] = ((x1 - x3) * z3x + (y1 - y3) * z3y)/3.0 + z3;
    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[10],
			 d->t[i].v[2], 1.0);
    ...
    /* C11 */
    d->t[i].ct[11] = ((x4 - x3) * z3x + (y4 - y3) * z3y)/3.0 + z3;
    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[11],
			 d->t[i].v[2], 1.0);
    ...
    /* C12 */
    d->t[i].ct[12] = ((x2 - x3) * z3x + (y2 - y3) * z3y)/3.0 + z3;
    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[12],
			 d->t[i].v[2], 1.0);
    ...
    /*
     * Constants of triangle (functions of shape)
     */
    pdx = x2 - x1;
    pdy = y2 - y1;
    ...
    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[13],
			 d->t[i].v[0], (theta1 - 1.0)/2.0);
    ct_add_weight(&d->t[i].nweights,
		  d->t[i].weight_refs,
		  d->t[i].weights[13],
		  d->t[i].v[1], -theta1/2.0);
    ct_combine_weight(d->t[i].nweights,
		      d->t[i].weights[13],
		      d->t[i].weights[5],
		      0.5);
    ...
    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[14],
			 d->t[i].v[1], (theta2 - 1.0)/2.0);
    ct_add_weight(&d->t[i].nweights,
		  d->t[i].weight_refs,
		  d->t[i].weights[14],
		  d->t[i].v[2], -theta2/2.0);
    ct_combine_weight(d->t[i].nweights,
		      d->t[i].weights[14],
		      d->t[i].weights[8],
		      0.5);
    ...
    ct_initialize_weight(d->t[i].nweights,
			 d->t[i].weight_refs,
			 d->t[i].weights[15],
			 d->t[i].v[2], (theta3 - 1.0)/2.0);
    ct_add_weight(&d->t[i].nweights,
		  d->t[i].weight_refs,
		  d->t[i].weights[15],
		  d->t[i].v[0], -theta3/2.0);
    ct_combine_weight(d->t[i].nweights,
		      d->t[i].weights[15],
		      d->t[i].weights[11],
		      0.5);
    ...
    d->t[i].ct[16] = (d->t[i].ct[15] + 
		      d->t[i].ct[5] + 
		      d->t[i].ct[13])/3.0;
    ct_initialize_weight_with_three(d->t[i].nweights,
				    d->t[i].weights[16],
				    d->t[i].weights[15],
				    d->t[i].weights[5],
				    d->t[i].weights[13]);
    ...
    /* for (k = 4; k <= 19; k ++) { */
    /*   dx = 0.0; */
    /*   for (j = 0; j < d->t[i].nweights; j ++) { */
    /* 	dx += d->t[i].weights[k][j] * d->p[d->t[i].weight_refs[j]].z; */
    /*   } */
    /*   printf("D Test: %2d %10.6f %10.6f\n", k, dx, d->t[i].ct[k]); */
    /* } */
				...
				double *weights)
{
  ...
  case 1:
    ...
    *npoints = d->t[t].nweights;
    for (i = 0; i < (*npoints); i ++) {
      indices[i] = d->t[t].weight_refs[i];
      if (indices[i] == d->t[t].v[1]) {
	weights[i] = v*v*v;
      } else if (indices[i] == d->t[t].v[0]) {
	weights[i] = u*u*u;
      } else {
	weights[i] = 0.0;
      }
    }
    ct_combine_weight(*npoints,
		      weights,
		      d->t[t].weights[19],
		      w*w*w);
    ct_combine_weight(*npoints,
    ...
    /* u = 0.0; */
    /* for (i = 0; i < (*npoints); i ++) { */
    /*   u += weights[i] * d->p[indices[i]].z; */
    /* } */
    /* printf("Case 1: %10.6f %10.6f\n", u, *z); */
					   ...
					   const double *weights3)
{
  ...
  for (i = 0; i < nweights; i ++) {
    weights[i] = (weights1[i] + weights2[i] + weights3[i])/3.0;
  }
