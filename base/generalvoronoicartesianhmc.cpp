...
int main(int argc, char *argv[])
{
  ...
  if (posterior) {
    pc.add(new HMC(*global_state, L, epsilon), 0.1);

    pc.add(new BirthGeneric(nmodels,
			    global_state->birthdeath_proposals,
			    global_state->birthdeath_position_proposals), 1.0);
    pc.add(new DeathGeneric(nmodels,
			    global_state->birthdeath_proposals,
			    global_state->birthdeath_position_proposals), 1.0);
  } else {
    pc.add(new HMC(*global_state, L, epsilon), 1.0);
    ...
    if (Pb > 0.0 || initialcells > 1) {
      pc.add(new Move(nmodels), 0.5);
    }
  ...
  double model_vmin = 0.0, model_vmax = 0.0;
  ...
  for (int i = 0; i < total; i ++) {
    ...
    global_state->models[0]->range(model_vmin, model_vmax);
	...
	printf("%3d ", m->nvaluecells());
      ...
      printf("Model %10.6f %10.6f\n", model_vmin, model_vmax);
    }
