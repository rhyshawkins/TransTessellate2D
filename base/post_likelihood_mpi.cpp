...
int main(int argc, char *argv[])
{
  ...
  mkrankpath(mpi_rank, nullptr, input, filename);
  chainhistoryreaderVoronoi reader(filename);
  ...
  mkrankpath(mpi_rank, nullptr, output, filename);
  FILE *fp_like = fopen(filename, "w");
  if (fp_like == NULL) {
  ...
  if (hierarchical_file) {
    mkrankpath(mpi_rank, nullptr, hierarchical_file, filename);
    fp_hierarchical = fopen(filename, "w");
  ...
  if (khistory_file) {
    mkrankpath(mpi_rank, nullptr, khistory_file, filename);
    fp_khistory = fopen(filename, "w");
    if (fp_khistory == NULL) {
