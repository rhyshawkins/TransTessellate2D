...
static char short_options[] = "i:I:o:x:X:y:Y:A:P:H:M:B:T:S:t:l:v:b:pC:z:Z:c:K:m:e:E:L:S:h";
static struct option long_options[] = {
  ...
  {"hmc-epsilon", required_argument, 0, 'E'},
  {"hmc-steps", required_argument, 0, 'L'},
  {"hmc-scale", required_argument, 0, 'S'},
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    case 'L':
      L = atoi(optarg);
      if (L <= 1) {
	fprintf(stderr, "error: no. steps must be 2 or greater\n");
	return -1;
      }
      break;

    case 'S':
      hmc_scale = atof(optarg);
      if (hmc_scale < 0.0) {
	fprintf(stderr, "error: hmc auto scale must be 0 or more\n");
	return -1;
      }
      break;
  ...
  HMC *hmc = new HMC(*global_state, L, epsilon);
  ...
  if (chain_rank == 0 && temperature == 1.0) {
  ...
  for (int i = 0; i < total; i ++) {
	...
	if (chain_rank == 0 && history != nullptr) {
	...
	INFO_LARGE_WRITE("Epsilon: %16.9e\n", hmc->get_epsilon());
    ...
    if (hmc_scale > 0.0) {
      hmc->adjust_epsilon(hmc_target, hmc_scale, hmc_adjust_steps);
    }
